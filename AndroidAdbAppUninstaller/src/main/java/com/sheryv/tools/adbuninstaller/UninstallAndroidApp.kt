...
@CommandLine.Command(
  name = "java -jar android-adb-app-uninstaller-all-*.jar",
  mixinStandardHelpOptions = true,
  description = [AdbUninstallCommand.DESCRIPTION],
  versionProvider = AdbUninstallCommand.VersionProvider::class,
...
class AdbUninstallCommand : Callable<Int> {
  companion object {
    ...
    const val DESCRIPTION = "%nThis app uninstalls applications on Android device connected to local machine. " +
  ...
  internal class VersionProvider : IVersionProvider {
    override fun getVersion(): Array<String> {
      return arrayOf(version().version, "Built at " + version().toTimestamp())
    }
