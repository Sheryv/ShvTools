...
operator fun KeyCodeCombination.plus(modifier: Modifier) {
  var sh = shift
  var c = control
  var a = alt
  var m = meta
  ...
  KeyCodeCombination(this.code, sh, c, a, m, s)
}
...
fun Menu.radiomenuitem(
  ...
  keyCombination?.apply { it.accelerator = this }
  it.properties["shvtools.toggleGroupValue"] = value ?: name
  graphic?.apply { it.graphic = graphic }
  op(it)
  this += it
}
...
fun ContextMenu.radiomenuitem(
  ...
  toggleGroup?.apply { it.toggleGroup = this }
  keyCombination?.apply { it.accelerator = this }
  properties["shvtools.toggleGroupValue"] = value ?: name
  graphic?.apply { it.graphic = graphic }
  op(it)
  this += it
}
