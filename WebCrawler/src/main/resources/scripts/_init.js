...
Object.keys(window).filter(k => k.match(/^\\?cdc_a.*/)).forEach(k => {
  delete window[k]
})
Object.keys(document).filter(k => k.match(/^\\?cdc_a.*/)).forEach(k => {
  delete document[k]
})
...
let shv = function () {
  ...
  function findAllIn(parent, childSelector = null, childIndex = null) {
    let parents = [parent];
    if (typeof parent === "string") {
      parents = find(parent);
    }

    return parents && parents.flatMap(p => {
      let children;
      if (childSelector == null) {
        children = [...p.childNodes.values()] || [];
      } else {
        children = [...p.querySelectorAll(childSelector)] || [];
      }

      if (childIndex != null && Number(childIndex) >= 0) {
  ...
  function findIn(parent, childSelector = null, childIndex = null, parentIndex = 0) {
    let elem = parent;
    if (typeof parent === "string") {
      elem = find(parent)[parentIndex];
    }
    let children;
    if (childSelector == null) {
      children = elem && [...elem.childNodes.values()] || [];
    } else {
      children = elem && [...elem.querySelectorAll(childSelector)] || [];
    }
  ...
  function click(selector, duration = 100) {
    let elem = selector;
    if (typeof selector === "string") {
      elem = findOne(selector);
    }
    vpp.move_to_element_and_click(elem, duration);
  }

  function hover(selector, duration = null) {
    let elem = selector;
    if (typeof selector === "string") {
      elem = findOne(selector);
    }
    if (duration == null || Number(duration) < 1) {
      duration = Math.floor(Math.random() * 300) + 200;
    }
    vpp.move_mouse_to_element(elem, duration);
  }

  function click_fast(selector) {
    click(selector, Math.floor(Math.random() * 300) + 200)
  }

  function click_slow(selector) {
    click(selector, Math.floor(Math.random() * 2000) + 1200)
  }
