package com.sheryv.tools.websitescraper.view.search
...
import com.sheryv.tools.websitescraper.service.videosearch.SearchItem
import com.sheryv.tools.websitescraper.service.videosearch.TmdbApi
import com.sheryv.tools.websitescraper.service.videosearch.TmdbEpisode
import com.sheryv.tools.websitescraper.utils.Utils
import com.sheryv.tools.websitescraper.utils.inBackground
...
class SearchWindow {
      ...
      doInBackground<Series, Any?>(
        lastSeries!!,
        {
          ...
          for (episode in lastSeries!!.episodes) {
            val size = getDownloadSize(episode.downloadUrl)
  ...
  private fun fillList(season: Series?) {
    ...
    for (i in seasonEpisodes.indices) {
      ...
      val row: JComponent = appendRow(
        episodes, String.format("%02d [%s] %s", episode.number, sizeString, episode.title), episode.downloadUrl,
        String::class.java, {
          if (it.isNotBlank()) {
            seasonEpisodes[i] = episode.copy(downloadUrl = it, errors = emptyList())
  ...
  private fun onVersionChanged(i: SearchItem) {
    ...
    doInBackground(i, {
      api.getTvEpisodes(i.id, season).episodes.map { ep: TmdbEpisode ->
        ...
        Episode(ep.name, ep.episodeNumber, "", "")
      }
    },
      { episodeList ->
  ...
  private fun showFileNamesList(series: Series?) {
    ...
    field.document.addDocumentListener(object : DocumentListener {
      ...
      override fun insertUpdate(e: DocumentEvent) {
        val collect = series!!.episodes.joinToString("\n") { it.generateFileName(series, field.text, settings) }
    ...
    createFilesBtn.addActionListener { e: ActionEvent? ->
      ...
      if (option == JFileChooser.APPROVE_OPTION) {
        ...
        series!!.episodes.map { ep: Episode ->
          ep.generateFileName(series, field.text, settings)
        }.forEach { f: String ->
          val dir =
  ...
  private fun loadSize(episode: Episode) {
    if (episode.downloadUrl.isBlank()) return
    setProgress(-1)
    doInBackground(
      episode,
      { getDownloadSize(it.downloadUrl) },
      {
        episode.lastSize = it
        fillList(lastSeries)
      },
